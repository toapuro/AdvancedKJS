...
import dev.toapuro.kubeextra.util.CommonUtil;
import dev.toapuro.kubeextra.util.ReflectionUtil;
...
public class AgentTransformer {
    ...
    public void retransformClass(Class<?> clazz, ClassFileTransformer classFileTransformer) {

        instrumentation.addTransformer(classFileTransformer, true);
        try {
            instrumentation.retransformClasses(clazz);
        } catch (UnmodifiableClassException e) {
            LOGGER.error("Could not transform class {}", clazz.getName());
        }
        instrumentation.removeTransformer(classFileTransformer);
    }
